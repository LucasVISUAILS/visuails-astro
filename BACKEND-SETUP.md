...
| Variable (optional) | `ALLOWED_ORIGIN_HOSTS` | see below — leave unset unless you hit the case it's for |

That was the complete list through task #271e. It was derived by reading every
`env.*` access in every file reachable from `functions/`, not from memory, so if
a new one ever appears in the code it will be missing from here rather than
silently undefined in production — this row is that promise being kept, not
broken: `ALLOWED_ORIGIN_HOSTS` is new, 2026-07-29, added for a real "Request
origin did not match" 403 on /admin and /account that Lucas hit in production.
`src/lib/admin.js`'s and `src/lib/account.js`'s Origin check (see either file's
header) compares the browser's `Origin` header against the exact host the
request came in on — correct for a single-hostname deployment, but Cloudflare
Pages can legitimately serve the same project under more than one hostname (an
apex/www pair, or the project's own `*.pages.dev` domain alongside the custom
one), and a same-site POST from any of those is not a forged request. Leave
this unset — the check behaves exactly as before. If the 403 page's new
diagnostic line ("Seen Origin: ..., Expected host: ...") shows a second,
legitimate hostname, set this to a comma-separated list of the extra host(s)
(not the primary one — that one is already covered) and redeploy.
