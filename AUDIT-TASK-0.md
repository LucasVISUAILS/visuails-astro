...
> **Addendum, 2026-07-29 (task #268/#270) — partially reversed.** Flag 8 shipped as written and held for months: `/models` 301'd to `/custom-models`, the standard roster lived only as `#standard` on that page. Lucas asked the standard-models entry point back — his framing was about visitors "getting lost in the information" on a page whose H1, hero and eyebrow all argue the exclusive case, with the shared roster buried mid-page and reachable only by a fragment link nobody types. That is a discoverability complaint, not a content one: the roster itself was never missing, exactly per this flag's recommendation.
>
> The fix does not undo the merge — it keeps the reasoning above (one exclusive-face story, told once, on `/custom-models`) and adds back a real front door for the other story (ten shared models, included in every drop, no setup fee) at `/models`. To do that without two pages indexing the same ten portraits as duplicate content: the roster moved out of `BrandModelPage.astro` into `src/data/models.js` as a shared source, `/models` now renders the full ten as its own page with its own framing, and `/custom-models`'s `#standard` block shrank to a four-model taste with a link to the rest. The `/models` → `/custom-models` 301 block in `public/_redirects` is removed; `/models` is a real route again. Section G's redirect map below (and the "SHIPPED" note against it) is stale as of this addendum — left in place rather than rewritten, per this file's own "flag deviations, keep history" discipline.
