...
db.prepare("INSERT INTO customers (id, email, brand) VALUES (5, 'eef@vijfde.test', 'VIJFDE')").run();
 ...
 * straks gemaakt wordt, dus hij hoort geen mail te krijgen.
 *
 * Klant 1 heeft er één zonder foto's. Die kan niet vastgezet worden en telt dus
 * niet mee: zijn week begint leeg en hij hoort bericht te krijgen.
 *
 * En klant 5 is er op 29 augustus 2026 bij gekomen, want daar zat het gat: een
 * item MÉT foto's dat nog een CONCEPT is. Dat telde mee als klaar zolang deze
 * telling alleen naar het uploadveld keek, en dan zwijgt deze taak precies bij
 * de klant die de mail het hardst nodig heeft — zijn lijst ziet er vol uit en
 * zijn week begint leeg. */
db.prepare("INSERT INTO plan_queue (customer_id, position, name, upload_batch, locked_at) VALUES (2, 0, 'Winterjas', 'batch-abc', '2026-08-01 10:00:00')").run();
...
// En klant 5: foto's erbij, maar nooit vastgezet. Een concept is geen werk.
db.prepare("INSERT INTO plan_queue (customer_id, position, name, upload_batch) VALUES (5, 0, 'Jas', 'batch-concept')").run();
...
/* ── EN DE KLANT MET ALLEEN CONCEPTEN WÉL — 29 augustus 2026 ─────────────────
   Dit is de regel die er sinds migratie 0035 bij hoort en die stil ontbrak: zijn
   lijst staat vol, mét foto's, en er wordt niets van gemaakt omdat hij niets
   heeft vastgezet. Precies de klant die deze mail nodig heeft. */
ok('en de klant met alleen concepten óók', gemaild.includes('eef@vijfde.test'));
ok('twee mails dus', gemaild.length, 2);
...
console.log('\néén mail per maand, ook als de taak elke nacht draait');
{
  ...
  ok('en er staat één geheugenrij per gemailde klant', rijen, 2);
}
