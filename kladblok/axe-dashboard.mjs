/* axe-core over het KLANTDASHBOARD. Dat wordt door Pages Functions gerenderd en
   ...
   en public/account.css gaan mee, want zonder die twee meet je een pagina die
   niemand krijgt. */
import { readFileSync, readdirSync, existsSync } from 'node:fs';
import { join, basename } from 'node:path';
import { chromium } from 'playwright';

const MAP = process.argv[2] || '/tmp/dash';
const ROOT = '/tmp/vb';
const AXE = readFileSync(join(ROOT, 'node_modules/axe-core/axe.min.js'), 'utf8');
const CSS = readFileSync(join(ROOT, 'public/account.css'), 'utf8');
...
for (const [w, h] of [[1280, 900], [420, 900]]) {
  ...
  await ctx.route('**/*', (route) => {
    ...
    if (/^\/account\/files\/(\d+)\//.test(u.pathname) || u.pathname.startsWith('/img/')) {
